...
import static com.zl.community.constant.Constants.UserConstant.BUTTON;
...
public class RbacAuthorityServiceConfig {
    ...
    public boolean  hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (userInfo instanceof UserDetails) {
            ...
            List<RoleEntity> roles = roleMapper.getUserRolesByUserId(userId);
            ...
            List<PermissionEntity> btnPerms = permissions.stream()
                    // 过滤页面权限
                    .filter(permission -> Objects.equals(permission.getType(), BUTTON))
                    // 过滤 URL 为空
            ...
            for (PermissionEntity btnPerm : btnPerms) {
                AntPathRequestMatcher antPathMatcher = new AntPathRequestMatcher(btnPerm.getUrl(), btnPerm.getMethod());

                System.out.println("request.getMethod() = " + request.getMethod());
                System.out.println("request.getRequestURI() = " + request.getRequestURI());
                System.out.println("btnPerm.getMethod() = " + btnPerm.getMethod());
                System.out.println("btnPerm.getUrl() = " + btnPerm.getUrl());
...
//     */
    private void checkRequest(HttpServletRequest request) {
        ...
        for (String uri : urlMapping.keySet()) {
            ...
            // 1：new AntPathRequestMatcher(uri,method) 这种方式可以直接判断方法是否匹配，因为这里把方法不匹配自定义抛出，所以使用第2种方式创建
            // 2：new AntPathRequestMatcher(uri) 这种方式不校验请求方法，只校验请求路径
            AntPathRequestMatcher antPathMatcher = new AntPathRequestMatcher(uri);
            if (antPathMatcher.matches(request)) {
                if (!urlMapping.get(uri)
                        .contains(currentMethod)) {
                    throw new SecurityException(ResultCode.NOT_FOUND_ERROR);
                } else {
                    return;
                }
            }
